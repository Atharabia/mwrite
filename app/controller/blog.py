...
class BlogController:
    ...
    async def get_blog(db: AsyncSession, *, blog_id: int) -> BlogDTO | None:
        result = await db.exec(
            select(BlogTable).where(BlogTable.id == blog_id))
        row = result.first()
        if row is None:
            return None
        return BlogDTO.model_validate(row, from_attributes=True)
    ...
    @staticmethod
    @session
    async def get_published_blog(db: AsyncSession,
                                 *,
                                 slug: str) -> BlogReaderDTO | None:
        result = await db.exec(
            select(BlogTable)
    ...
    @staticmethod
    @session
    async def publish_blog(db: AsyncSession,
                           *,
                           blog: BlogCreateDTO) -> BlogDTO:
    ...
    @staticmethod
    @session
    async def update_blog(db: AsyncSession,
                          *,
                          blog_id: int,
                          update: BlogUpdateDTO) -> BlogDTO | None:
        result = await db.exec(select(BlogTable)
