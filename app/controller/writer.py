...
from app.database import WriterTable, RoleTable, WriterRoleTable
from app.database import session
from app.models.dto import WriterDTO, WriterWithRolesDTO
...
class WriterController:
    ...
    @staticmethod
    @session
    async def get_writer_roles(db: AsyncSession, *, writer_id: int) -> list[str]:
        result = await db.exec(
            select(RoleTable.name)
            .join(WriterRoleTable, WriterRoleTable.role_id == RoleTable.id)
    ...
    async def list_writers(db: AsyncSession) -> list[WriterWithRolesDTO]:
        ...
        roles_result = await db.exec(
            ...
            .where(WriterRoleTable.writer_id.in_(writer_ids))
        )
    ...
    async def create_writer(
        ...
        *,
        ...
        roles_result = await db.exec(
            select(RoleTable).where(RoleTable.name.in_(role_names)))
    ...
    async def update_writer(
        ...
        *,
        ...
        if email is not None:
            # Check the email isn't taken by another writer
            dup = await db.exec(
